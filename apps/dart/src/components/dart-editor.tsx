...
import { lintDart } from "../lib/linter";
...
export function DartEditor({ defaultCode = DEFAULT_CODE, height = "300px" }: DartEditorProps) {
  ...
  const lintExtension = useMemo(
    () => createEditorLinter(lintDart, setProblems),
    [],
  );
  ...
  const runCode = useCallback(async () => {
    ...
    try {
      const result = await executeCode("dart", code);
      const parts: string[] = [];
