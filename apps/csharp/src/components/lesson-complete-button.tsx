...
export function LessonCompleteButton({ categoryId, lessonId }: LessonCompleteButtonProps) {
  ...
  const toggle = () => {
    if (!complete) {
      ...
      setComplete(true);
      window.dispatchEvent(new StorageEvent("storage", { key: "csharplearn-progress" }));
    } else {
      const STORAGE_KEY = "csharplearn-progress";
      try {
  ...
  return (
    ...
    >
      <span className={`w-5 h-5 rounded-full flex items-center justify-center border-2 shrink-0 transition-colors ${
        complete ? "border-purple-500 bg-purple-500" : "border-gray-600"
      }`}>
        {complete && (
