...
const NAV_GROUPS = [
  {
    ...
    items: [
      ...
      { href: "/learn/methods", label: "メソッド" },
    ],
  },
  {
    label: "データ構造",
    items: [
      { href: "/learn/arrays", label: "配列・リスト" },
  ...
  },
  {
    ...
    items: [
      ...
      { href: "/learn/inheritance", label: "継承・IF" },
  ...
  },
  {
    label: "モダンC#",
    items: [
      { href: "/learn/linq", label: "LINQ" },
      { href: "/learn/async", label: "非同期処理" },
      { href: "/learn/delegates", label: "デリゲート" },
    ],
  },
  {
    label: "Web開発",
    items: [
      { href: "/learn/aspnet", label: "ASP.NET Core" },
      { href: "/learn/webapi", label: "Web API" },
      { href: "/learn/blazor", label: "Blazor" },
    ],
  },
  {
    label: "データアクセス",
    items: [
      { href: "/learn/efcore", label: "EF Core" },
      { href: "/learn/database", label: "データベース" },
    ],
  },
  {
    label: "テスト・品質",
    items: [
      { href: "/learn/xunit", label: "xUnit" },
  ...
  },
  {
    ...
    items: [
      ...
      { href: "/learn/patterns", label: "パターンマッチング" },
    ],
  },
  {
    label: "スキルアップ",
    items: [
      ...
      { href: "/learn/dotnet", label: ".NETエコシステム" },
    ],
  },
];
    ...
    <div ref={ref} className="relative">
      <button
        ...
        className={`flex items-center gap-1 px-2.5 py-1.5 rounded text-xs font-medium whitespace-nowrap transition-colors ${
          hasActiveItem ? "bg-purple-500/20 text-purple-400" : "text-gray-400 hover:text-gray-100 hover:bg-gray-800"
        }`}
      >
        {group.label}
          ...
          {group.items.map((item) => (
            <Link key={item.href} href={item.href} onClick={() => setOpen(false)}
              className={`block px-3 py-2 text-xs transition-colors ${
                isActive(item.href) ? "bg-purple-500/20 text-purple-400" : "text-gray-300 hover:text-gray-100 hover:bg-gray-700"
              }`}
            >{item.label}</Link>
          ))}
        </div>
