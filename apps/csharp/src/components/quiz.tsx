...
export function Quiz({ questions, color = "purple" }: QuizProps) {
  const [selected, setSelected] = useState<(number | null)[]>(
    Array(questions.length).fill(null)
  );
  const [submitted, setSubmitted] = useState(false);

  const accentClass = colorMap[color] ?? colorMap.purple;
    ...
    <div className="mt-8 space-y-6">
      ...
      {!submitted ? (
        <button
          ...
          className="w-full py-3 rounded-xl font-semibold text-sm transition-colors bg-purple-600 hover:bg-purple-500 disabled:bg-gray-700 disabled:text-gray-500 disabled:cursor-not-allowed text-white"
        >
