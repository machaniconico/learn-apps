...
export function JavaEditor({ defaultCode = DEFAULT_CODE, height = "300px", expectedOutput }: JavaEditorProps) {
      ...
      {showOutput && !expectedOutput && (
        <div className="border-t border-gray-700">
          ...
          <div className="px-4 py-3 text-sm text-gray-400 bg-gray-950">
            <p>Javaはブラウザ上では実行できません。コードの動作を確認するには、JDKをインストールしてローカル環境で実行してください。</p>
            <p className="mt-1 text-xs text-gray-500"><code className="bg-gray-800 px-1.5 py-0.5 rounded">javac Main.java && java Main</code> コマンドで実行できます。</p>
          </div>
        </div>
      )}
    </div>
