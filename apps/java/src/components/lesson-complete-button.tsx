...
export function LessonCompleteButton({ categoryId, lessonId }: LessonCompleteButtonProps) {
  ...
  const toggle = () => {
    if (!complete) {
      ...
      setComplete(true);
      window.dispatchEvent(new StorageEvent("storage", { key: "javalearn-progress" }));
    } else {
      const STORAGE_KEY = "javalearn-progress";
      try {
  ...
  return (
    ...
    >
      <span className={`w-5 h-5 rounded-full flex items-center justify-center border-2 shrink-0 transition-colors ${
        complete ? "border-orange-500 bg-orange-500" : "border-gray-600"
      }`}>
        {complete && (
