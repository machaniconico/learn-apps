    ...
    return (
      <div className="min-h-screen bg-gray-950 flex items-center justify-center px-4">
        <div className="text-center">
          ...
          <Link href="/final-exam" className="bg-indigo-500 hover:bg-indigo-400 text-white font-semibold px-6 py-3 rounded-lg transition-colors">最終試験を受ける</Link>
        </div>
      </div>
    );
  }
        ...
        <div className="bg-white text-gray-900 rounded-2xl p-12 shadow-2xl border-4 border-amber-400 print:shadow-none print:border-2">
          <div className="text-center space-y-8">
            ...
            <div className="py-4">
              ...
              <p className="text-2xl font-bold text-indigo-600 border-b-2 border-indigo-200 inline-block px-8 pb-1">{userName || "学習者"}</p>
              <p className="text-gray-500 text-sm mt-3">が以下のコースを修了したことを証明します</p>
            </div>

            <div className="bg-gray-50 rounded-xl p-6">
              <h4 className="text-xl font-bold text-gray-800 mb-1">PHPLearn - PHP言語学習</h4>
              <p className="text-gray-500 text-sm">全25カテゴリ・163レッスン</p>
            </div>

            <div className="grid grid-cols-2 gap-8">
              <div><p className="text-sm text-gray-500">最終試験スコア</p><p className="text-2xl font-bold text-indigo-600">{examResult.score}点</p></div>
              <div><p className="text-sm text-gray-500">取得日</p><p className="text-lg font-semibold text-gray-700">{dateStr}</p></div>
            </div>

            <div className="pt-4 border-t border-gray-200">
              <p className="text-xs text-gray-400">PHPLearn Learning Platform</p>
            </div>
          </div>
        </div>

        <div className="mt-6 flex justify-center gap-4">
          <button onClick={() => window.print()} className="bg-indigo-500 hover:bg-indigo-400 text-white font-semibold px-6 py-3 rounded-lg transition-colors print:hidden">
            印刷 / PDF保存
          </button>
