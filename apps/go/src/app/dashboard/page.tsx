            ...
            {editingName ? (
              <div className="flex items-center gap-2">
                <input
                  ...
                  placeholder="お名前を入力"
                  className="bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-gray-100 text-sm focus:outline-none focus:border-cyan-500"
                  autoFocus
                />
                <button
                  onClick={handleSaveName}
                  className="bg-cyan-500 hover:bg-cyan-400 text-gray-950 font-semibold px-3 py-2 rounded-lg text-sm transition-colors"
                >
                  保存
                </button>
                <button
            ...
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
              ...
              {examResult ? (
                <p className="text-2xl font-bold text-gray-100">
                  ...
                  {examPassed && (
                    <span className="ml-2 text-sm font-medium text-cyan-400">合格</span>
                  )}
                </p>
              ) : (
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {categoryStats.map((cat) => {
              const dotColor = colorDotMap[cat.color] ?? colorDotMap.cyan;
              const barColor = colorBarMap[cat.color] ?? colorBarMap.cyan;
