...
export function Quiz({ questions, color = "orange" }: QuizProps) {
  const [selected, setSelected] = useState<(number | null)[]>(
    Array(questions.length).fill(null)
  );
  const [submitted, setSubmitted] = useState(false);

  const accentClass = colorMap[color] ?? colorMap.orange;
  const btnColor = buttonColorMap[color] ?? buttonColorMap.orange;
