...
import { lintSwift } from "../lib/linter";
...
const DEFAULT_CODE = `// Swiftコードを確認してみましょう！
...
export function SwiftEditor({ defaultCode = DEFAULT_CODE, height = "300px" }: SwiftEditorProps) {
  ...
  const lintExtension = useMemo(
    () => createEditorLinter(lintSwift, setProblems),
    [],
  );
  ...
  const runCode = useCallback(async () => {
    ...
    try {
      const result = await executeCode("swift", code);
      const parts: string[] = [];
    ...
    <div className="rounded-xl border border-gray-700 overflow-hidden bg-gray-900">
      <div className="flex items-center justify-between px-4 py-2 bg-gray-800 border-b border-gray-700">
        <div className="flex items-center gap-2">
          <span className="text-orange-400 font-mono text-sm font-semibold">Swift</span>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={runCode}
            disabled={loading}
            className="flex items-center gap-1.5 bg-orange-600 hover:bg-orange-500 disabled:bg-orange-800 disabled:cursor-not-allowed text-white text-sm font-medium px-3 py-1.5 rounded transition-colors"
          >
            {loading ? (
              <>
      ...
      <CodeMirror value={code} height={height} extensions={[javascript(), basicSetup, lintExtension, ...(showHint ? [placeholderExtension] : [])]} theme={oneDark} onChange={setCode} className="text-sm" />
