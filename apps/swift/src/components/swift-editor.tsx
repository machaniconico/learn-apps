...
export function SwiftEditor({ defaultCode = DEFAULT_CODE, height = "300px", expectedOutput }: SwiftEditorProps) {
    ...
    <div className="rounded-xl border border-gray-700 overflow-hidden bg-gray-900">
      <div className="flex items-center justify-between px-4 py-2 bg-gray-800 border-b border-gray-700">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setShowOutput(true)} className="flex items-center gap-1.5 bg-orange-600 hover:bg-orange-500 text-white text-sm font-medium px-3 py-1.5 rounded transition-colors">
      ...
      {showOutput && !expectedOutput && (
        <div className="border-t border-gray-700">
          ...
          <div className="px-4 py-3 text-sm text-gray-400 bg-gray-950">
            <p>Swiftはブラウザ上では実行できません。コードの動作を確認するには、XcodeまたはSwift Playgroundをご利用ください。</p>
            <p className="mt-1 text-xs text-gray-500"><code className="bg-gray-800 px-1.5 py-0.5 rounded">swift main.swift</code> コマンドで実行できます。</p>
          </div>
        </div>
      )}
    </div>
