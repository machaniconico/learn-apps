...
export function LessonCompleteButton({ categoryId, lessonId }: LessonCompleteButtonProps) {
  ...
  const toggle = () => {
    if (!complete) {
      ...
      setComplete(true);
      window.dispatchEvent(new StorageEvent("storage", { key: "swiftlearn-progress" }));
    } else {
      const STORAGE_KEY = "swiftlearn-progress";
      try {
  ...
  return (
    ...
    >
      <span className={`w-5 h-5 rounded-full flex items-center justify-center border-2 shrink-0 transition-colors ${
        complete ? "border-cyan-500 bg-cyan-500" : "border-gray-600"
      }`}>
        {complete && (
