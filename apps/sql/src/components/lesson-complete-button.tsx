...
export function LessonCompleteButton({ categoryId, lessonId }: LessonCompleteButtonProps) {
  ...
  const toggle = () => {
    if (!complete) {
      ...
      setComplete(true);
      // Notify other tabs
      window.dispatchEvent(new StorageEvent("storage", { key: "pythonlearn-progress" }));
    } else {
      // Mark incomplete by removing from localStorage
      const STORAGE_KEY = "pythonlearn-progress";
      try {
  ...
  return (
    ...
    >
      <span className={`w-5 h-5 rounded-full flex items-center justify-center border-2 shrink-0 transition-colors ${
        complete ? "border-green-500 bg-green-500" : "border-gray-600"
      }`}>
        {complete && (
