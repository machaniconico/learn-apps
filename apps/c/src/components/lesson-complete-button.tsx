...
export function LessonCompleteButton({ categoryId, lessonId }: LessonCompleteButtonProps) {
  ...
  const toggle = () => {
    if (!complete) {
      ...
      setComplete(true);
      window.dispatchEvent(new StorageEvent("storage", { key: "clearn-progress" }));
    } else {
      const STORAGE_KEY = "clearn-progress";
      try {
  ...
  return (
    ...
    >
      <span className={`w-5 h-5 rounded-full flex items-center justify-center border-2 shrink-0 transition-colors ${
        complete ? "border-teal-500 bg-teal-500" : "border-gray-600"
      }`}>
        {complete && (
